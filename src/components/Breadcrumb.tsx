...
const Links: React.FC<Props> = ({ breadcrumb }) => {
  return breadcrumb.map(item => {
    if (typeof item === "string") {
      return (
        <li key={ item } className="text-sm pl-2 capitalize leading-normal dark:text-slate-300 before:float-left before:pr-2 before:text-gray-600 before:content-['/']">
          { item }
        </li>
      )
    }
  ...
  <nav>
    <ol className="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
      <li className="text-sm leading-normal">
        <a className="opacity-50 dark:text-slate-200">Pages</a>
      </li>
